...
SYS_CONVERSATION = """你是面试实时助手。请用第一人称、口语化、自然的中文，给出可直接说出口的参考回答。
要求：先给结论，再补 1-2 句理由；总长 2-4 句；不要堆术语和套话；能结合候选人简历经历更好。
只输出参考回答本身，不要解释你在做什么。"""

SYS_STRUCTURED = """你是公考结构化面试答题助手。请按题型套用标准框架作答，输出"分点骨架(一是/二是/三是)+过渡语"，书面、正式、条理清晰。
常见题型与框架：
...
def build(mode: str, question: str):
    sys = SYS_STRUCTURED if mode == "structured" else SYS_CONVERSATION
    ctx = ""
    if config.RESUME_TEXT:
        ctx += f"\n【候选人简历】\n{config.RESUME_TEXT}\n"
    if config.COMPANY_TEXT:
        ctx += f"\n【应聘公司】\n{config.COMPANY_TEXT}\n"
    if ctx:
        sys = sys + "\n\n（以下为参考资料，作答时自然结合，不要照搬）" + ctx
    return sys, question
