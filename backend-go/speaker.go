...
// 每个 Session 持有自己的 sid（uuid），随请求头 X-Session-Id 发出。
...
func NewSidecarSpeaker(url, sid string) *SidecarSpeaker {
	return &SidecarSpeaker{url: url, sid: sid, cli: &http.Client{Timeout: 3 * time.Second}}
}
...
func (s *SidecarSpeaker) call(path string, pcm []int16) (map[string]any, error) {
	...
	req.Header.Set("X-Session-Id", s.sid)
...
// makeSpeaker：SPEAKER_SIDECAR_URL 为空 → NoopSpeaker（不分离）。
func makeSpeaker(cfg Config, sid string) Speaker {
	if cfg.SpeakerSidecarURL != "" {
		return NewSidecarSpeaker(cfg.SpeakerSidecarURL, sid)
	}
	return NoopSpeaker{}
}
